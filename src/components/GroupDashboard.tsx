...
import { Empresa, ResultadoDiagnostico, SectorType, SemaforoColor } from '@/types';
    ...
    <div className={`min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans transition-all ${isFullScreen ? 'p-6' : ''}`}>
      ...
      <div className="max-w-7xl w-full mx-auto p-4 md:p-6 space-y-6 flex-1 flex flex-col">
        ...
        <div className="bg-slate-800/80 backdrop-blur border border-slate-700 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Córdoba IA – Grupo 1 · Resultados Consolidados Anónimos</p>
            </div>
          </div>
        ...
        {esAnonimoInsuficiente ? (
          ...
          /* 7 Dashboard Blocks Grid */
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* Bloque 1: Avance en Vivo */}
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
              ...
              {revelados.b1 ? (
                ...
                <div className="py-8 text-center text-slate-500 text-xs font-semibold italic">Bloque Bloqueado (Modo Presentación)</div>
              )}
            </div>

            {/* Bloque 2: El Balde del Grupo */}
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
              <div className="flex items-center justify-between border-b border-slate-700/60 pb-3 mb-4">
                <div className="flex items-center gap-2 font-bold text-slate-200 text-sm">
                  <PieChart className="w-4 h-4 text-[#CCBBEE]" />
                  <span>2. El Balde del Grupo</span>
                </div>
                <button
                  onClick={() => toggleRevelar('b2')}
                  className="text-xs text-slate-400 hover:text-white flex items-center gap-1 cursor-pointer"
                >
                  {revelados.b2 ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5 text-[#ED7D31]" />}
                  <span>{revelados.b2 ? 'Ocultar' : 'Revelar'}</span>
                </button>
              </div>
              ...
              {revelados.b4 ? (
                <div className="space-y-2 text-center py-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Media de <span className="text-white font-bold">{avgHoras} horas/semana</span> por empresa consumidas en tareas Zombi y Grasa.
                  </p>
                </div>
              ) : (
                <div className="py-8 text-center text-slate-500 text-xs font-semibold italic">Bloque Bloqueado (Modo Presentación)</div>
            ...
            {/* Bloque 7: Alertas de Coherencia (Facilitador Privado - NUNCA SE PROYECTA) */}
            {!isFullScreen && (
              <div className="bg-slate-800/60 border border-slate-700/80 rounded-2xl p-5 shadow-lg md:col-span-2 lg:col-span-3">
